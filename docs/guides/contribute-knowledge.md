...
4. Add a citation when the new fact supports or contradicts an existing entry. Cite across planes freely: an entry on your open contribution branch may cite one already merged to `main` — this resolves correctly and the edge goes live in `main`'s DAG when your contribution merges. (Before bug.5024 this silently 500'd; it now works, so don't avoid citing merged atoms from a long-lived branch.)
5. Write a new atomic entry only when no existing entry fits — and nearly always cite at least one existing entry in the same edit (`supports`/`extends`/`contradicts`/`supersedes`). A new atom should compound onto the graph, not land as an island; recall almost always surfaces a parent or sibling to link.
...
- A new atom with zero citation edges — the island failure. A new entry should nearly always cite a parent/sibling recall surfaced (cross-plane to merged atoms works); islands don't compound and leave the hub a flat document store.
